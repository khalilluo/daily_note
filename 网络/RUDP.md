...
[![Lcmskq.png](https://s1.ax1x.com/2022/04/21/Lcmskq.png)](https://imgtu.com/i/Lcmskq)

#### 实时通信中什么是 “可靠”
不同的需求场景对可靠的需求是不一样的，我们在这里总体归纳为三类定义：
...
不管哪类场景，都是要保证可靠性，也就是质量，
那么在 UDP 之上怎么实现可靠呢？答案就是重传


#### 重传模式
 **RUDP 基本框架**
 [![LcMNUs.png](https://s1.ax1x.com/2022/04/21/LcMNUs.png)](https://imgtu.com/i/LcMNUs)
...
##### 定时重传
定时重传很好理解，就是发送端如果在发出数据包（T1）时刻一个 RTO (Retransmission Timeout)之后还未收到这个数据包的 ACK 消息，那么发送端就重传这个数据包。这种方式依赖于接收端的 ACK 和 RTO，容易产生误判，主要有两种情况：
...
[![LcJfTU.png](https://s1.ax1x.com/2022/04/21/LcJfTU.png)](https://imgtu.com/i/LcJfTU)
...
**应用到 RUDP 上示意图如下**
[![LcJ4kF.png](https://s1.ax1x.com/2022/04/21/LcJ4kF.png)](https://imgtu.com/i/LcJ4kF)
...
[![LcJbOx.png](https://s1.ax1x.com/2022/04/21/LcJbOx.png)](https://imgtu.com/i/LcJbOx)
...
![[Pasted image 20220421190338.png]]
...
**传输模型如下：**
[![LcYMXq.png](https://s1.ax1x.com/2022/04/21/LcYMXq.png)](https://imgtu.com/i/LcYMXq)
...
说到实时音视频传输就必然会想到 开源实时音视频工程 WebRTC，在 WebRTC 中对于视频传输也实现了一个拥塞控制算法 (GCC)，WebRTC 的 GCC 是一个基于发送端丢包率和接收端延迟带宽统计的拥塞控制，而且是一个尽力可靠的传输算法，在传输的过程中如果一个报文重发太多次后会直接丢弃，这符合视频传输的场景

GCC 的发送端会根据丢包率和一个对照表来 pacing rate，当 loss <2% 时，会加大传输带宽，当 loss>=2% &&loss <10%，会保持当前码率，当 loss>=10%，会认为传输过载，进行调小传输带宽。

GCC 的接收端是根据数据到达的延迟方差和大小进行 KalmanFilter 进行带宽逼近收敛，具体的细节不介绍了，请查看：《[WebRTC 视频接收缓冲区基于 KalmanFilter 的延迟模型](https://www.jianshu.com/p/bb34995c549a)》。
...
**如下图：**
[![LcYDHK.png](https://s1.ax1x.com/2022/04/21/LcYDHK.png)](https://imgtu.com/i/LcYDHK)
...
[![LcYLgs.png](https://s1.ax1x.com/2022/04/21/LcYLgs.png)](https://imgtu.com/i/LcYLgs)
