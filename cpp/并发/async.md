...
future对象析构的时候，可能发生阻塞（即确保线程已经完成）
...
```c++
void doSomethings(int par1, int par2){
    // ... do some Time-consuming things
}

std::vector<std::furture<void>> s_vecFur;

for (auto && path, filepaths){
    // 此处必须保存返回的future，否则会因为返回的future析构阻塞导致不能异步
    s_vecFur.push_back(std::async(std::Launch::async, doSomethings, 1, 2));
}
```
